...
pairs(data_num, diag.panel = panel.hist, upper.panel = panel.cor, 
      lower.panel = panel.smooth, colour = "#4ea5ff")
```
...
```{r total_calories, fig.height=4, fig.width=7, echo=FALSE}
...
```{r total_sugars, fig.height=4, fig.width=7, echo=FALSE}
par(mfrow = c(1, 1), mar = c(8, 4, 2, 2))
...
```{r fat_comparison_, fig.height=2.5, fig.width=6, echo=FALSE}
...
plot(total_fat_density, col = "#4ea5ff",
     ...
     xlim = range(data_cleaned$Total_Fat, data_cleaned$Trans_Fat), lwd = 2, lty = 1)
...
```{r backward_elimination_, include=FALSE}
backward_model <- step(lm_model, direction = "backward")
```

```{r backward_elimination, fig.height=4, fig.width=7}
par(mfrow = c(2, 2), mar = c(2, 2, 2, 2))
plot(backward_model)
...
```{r comparison, fig.height=4, fig.width=7, echo=FALSE}
...
```{r vif, fig.height=4, fig.width=7, echo=FALSE}
...
```{r backward_elimination_log_, include=FALSE}
...
```{r manula_backward_elimination_log, fig.height=4, fig.width=7}
...
```{r backward_elimination_log_2_, include=FALSE}
