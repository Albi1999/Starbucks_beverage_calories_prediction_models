...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{vif}\NormalTok{(lm\_model)}
\end{Highlighting}
\end{Shaded}
...
p-value is less than 0.05 We have a problem with multicollinearity, the
VIF values are high for some variables, so we have to act on the data to
solve this problem
...
\FunctionTok{summary}\NormalTok{(mod\_log\_tr)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{AIC}\NormalTok{(mod\_log\_tr)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{BIC}\NormalTok{(mod\_log\_tr)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{vif}\NormalTok{(mod\_log\_tr)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(mod\_lasso)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(mod\_ridge)}
\end{Highlighting}
\end{Shaded}
...
The model is the lasso because it has the lower value for R\^{}2 and
MSE.
...
\CommentTok{\# Calculate the R{-}squared value for the lasso regression model on the test data}
...
\NormalTok{lasso\_r\_squared\_test}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calculate the MSE for the lasso regression model on the test data}
\CommentTok{\# The MSE is a measure of the average squared difference }
\CommentTok{\# between the predicted and actual values}
...
test data The points are close to the diagonal line, indicating that the
model is making accurate predictions The R-squared value and MSE are
used to evaluate the performance of the model The R-squared value is
0.99, indicating that the model explains 99\% of the variance in the
data
